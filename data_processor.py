...
from processorlib.plain_content import get_plain_content
from processorlib.defend_content import get_defend_content
...
if __name__ == '__main__':
    ...
    with open('value_' + date + '.csv', 'w', encoding = 'big5', newline='\n') as csvfile:
        ...
        writer.writerow(['案件編號', '辯論終結日期', '原告', '原告代表人', '原告訴訟代理人', '被告', '被告代表人', '被告訴訟代理人', '不服訴願日期', '判決結果', '判決日期', '第幾庭', '審判長及法官', '事實字數', '原告字數', '被告字數', '判斷字數', '原告搜尋結果', '被告搜尋結果', '判斷搜尋結果', '補徵稅額'])

        for f_num in range(int(total)):
            file = open('data_' + date + '/' + str(f_num + 1) + '.txt', 'r', encoding = 'utf8')
            verdict = file.read().replace('　', '').replace(' ', '').replace('\ue4fd', ' ').replace('\ue3f1', '').replace('\u6052', '?')
            ...
            if type_line.find('訴字') != -1 or type_line.find('訴更') != -1:
                ...
                #plain_content = get_plain_content_simple(verdict, date, f_num + 1)
            ...
            if type_line.find('簡上') == -1:
                ...
                #plain_content = get_plain_content_litigation(verdict, date, f_num + 1)
            ...
            plain_content, plain_content_num = get_plain_content(verdict, date, f_num + 1)
            defend_content, defend_content_num = get_defend_content(verdict, date, f_num + 1)
            ...
            output = [f_num + 1, end_date, plaintiff, plain_represent, plain_attorney, defendant, defend_represent, defend_attorney, unsatisfied_date, result, judgment_date, court_num, judge, reason_content_num, plain_content_num, defend_content_num, result_content_num, plain_search, defend_search, result_search, money_inquiry]
